...
local Library do
    ...
    local Themes = {
		["Preset"] = {
			["Background"] = FromRGB(13, 12, 14),           -- Nền chính: đen pha xanh rêu
			["Border"] = FromRGB(31, 28, 32),               -- Viền
			["Inline"] = FromRGB(32, 30, 34),               -- Giữa các phần tử
			["Hovered Element"] = FromRGB(62, 50, 61),      -- Hover: nhấn xanh tươi
			["Page Background"] = FromRGB(19, 18, 20),      -- Nền trang phụ
			["Outline"] = FromRGB(69, 60, 74),              -- Outline
			["Element"] = FromRGB(36, 35, 42),              -- Các khối giao diện
			["Gradient"] = FromRGB(207, 180, 240),          -- Gradient nhẹ, xanh tươi
			["Text"] = FromRGB(237, 210, 246),              -- Chữ sáng, hơi ngà
			["Text Stroke"] = FromRGB(0, 0, 0),
			["Placeholder Text"] = FromRGB(175, 160, 174),
			["Accent"] = FromRGB(134, 112, 242)             -- 🎯 Matcha tươi
		}
    }
...
local updateDelay = .1  -- Default update delay (seconds)
...
local function getPing()
    ...
    if pingStat then
        ...
        return 0  -- Fallback if ping stat is unavailable
    end
end
...
local function calculateFPS()
    ...
    while frameTimes[1] and currentTime - frameTimes[1] > 1 do
...
local function updateWatermark()
    ...
    local ping = getPing()
    Items["Text"].Instance.Text = string.format("%s | %d fps | %d ms", Name, currentFPS, ping)
end

-- Start the update loop
local updateConnection = nil
local function startWatermarkUpdates(delay)
    if updateConnection then
        updateConnection:Disconnect()
    end
    updateDelay = delay
    lastUpdateTime = tick()
    frameTimes = {tick()}  -- Reset frame times
...
local Watermark = Library:Watermark("ver. 0.31b-28122025 ")
local KeybindList = Library:KeybindList()

-- Define the UI pages
local HadalPage = Window:Page({ Name = "The Hadal Blacksite", SubPages = true })
local RavePage = Window:Page({ Name = "The Raveyard", Columns = 1 })
local HuntedPage = Window:Page({ Name = "The Hunted", Columns = 1 })
local VisualPage = Window:Page({ Name = "Visual", Columns = 1 })
...
local rooms = gameplayFolder:WaitForChild("Rooms")
local monstersFolder = gameplayFolder:WaitForChild("Monsters")

-- Shared anti-feature monitoring
local antiConnections = {}
local antiEnabled = {
    Pandemonium = false, Eyefestation = false, Firewall = false, Abomination = false, Void = false,
    Mine = false, RemoteEvent = false, Turrets = false, Searchlights = false, Candles = false,
    Dweller = false, Baldi = false, Bouncer = false, SkeletonHead = false, Corpse = false, Divine = false,
}

local entities = {
    Angler = true, Chainsmoker = true, Pinkie = true, Blitz = true, Froger = true, Pandemonium = true,
    RidgeAngler = true, RidgeChainsmoker = true, RidgePinkie = true, RidgeBlitz = true, RidgeFroger = true,
    RidgePandemonium = true, ["A60"] = true, DiVineRoot = true
}
...
local safePart = nil
local angler = nil
local checkingEnemies = false
local enemyCheckConn = nil
local enemyRemoveConn = nil

-- Reusable functions
local function destroyInstance(instance, name)
    if instance then
        pcall(function() instance:Destroy() end)
    end
end

local function scanAndDestroy(parent, targetNames, enabledKey)
    if not antiEnabled[enabledKey] then return end
    for _, child in pairs(parent:GetChildren()) do
        if typeof(targetNames) == "table" then
            for _, name in pairs(targetNames) do
                if child.Name == name then destroyInstance(child, name) end
            end
        elseif child.Name == targetNames then
            destroyInstance(child, targetNames)
        end
    end
end

local function setupDescendantMonitor(parent, targetNames, enabledKey)
    local conn = parent.DescendantAdded:Connect(function(child)
        if antiEnabled[enabledKey] and (typeof(targetNames) == "table" and table.find(targetNames, child.Name) or child.Name == targetNames) then
            destroyInstance(child, child.Name)
        end
    end)
...
local function startLoop(name, callback, interval)
...
-- Hadal Blacksite Page
do
    -- Anti-Features Subpage
    local AntiFeaturesSubPage = HadalPage:SubPage({ Name = "Anti-Features", Columns = 1 })

    do
        local AntiFeaturesSection = AntiFeaturesSubPage:Section({ Name = "Anti-Features", Side = 1 })

        -- Anti Pandemonium
        AntiFeaturesSection:Toggle({
            Name = "Anti Pandemonium",
            Flag = "AntiPandemonium",
            Default = false,
            Callback = function(Value)
            antiEnabled.Pandemonium = Value
        if Value then
            scanAndDestroy(workspace, {"Pandemonium", "RidgePandemonium"}, "Pandemonium")
            setupDescendantMonitor(workspace, {"Pandemonium", "RidgePandemonium"}, "Pandemonium")
            startLoop("PandemoniumCheck", function() scanAndDestroy(workspace, {"Pandemonium", "RidgePandemonium"}, "Pandemonium") end, 0.5)
        else
            stopLoop("PandemoniumCheck")
        end
    end
        })
        ...
        AntiFeaturesSection:Toggle({
            ...
            Default = false,
            Callback = function(Value)
            antiEnabled.Eyefestation = Value
        if Value then
            -- Clear previous conns if any
            for _, conn in pairs(eyefestationConns) do
                conn:Disconnect()
            end
            eyefestationConns = {}
            ...
            for _, room in pairs(rooms:GetChildren()) do
                ...
                if interactables then
                    local eyefestationSpawn = interactables:FindFirstChild("EyefestationSpawn")
                    if eyefestationSpawn then
                        destroyInstance(eyefestationSpawn, "Eyefestation")
                    end
                    local eyefestationRoot = interactables:FindFirstChild("EyefestationRoot")
                    if eyefestationRoot then
                        destroyInstance(eyefestationRoot, "Eyefestation")
                    end
                end
            end
            -- Monitor new ones via descendant added
            local conn1 = rooms.DescendantAdded:Connect(function(child)
                task.spawn(function()
                    task.wait(0.1)
                    if antiEnabled.Eyefestation and child.Parent and child.Parent.Name == "Interactables" and (child.Name == "EyefestationSpawn" or child.Name == "EyefestationRoot") then
                        destroyInstance(child, "Eyefestation")
                    end
                end)
            end)
            table.insert(eyefestationConns, conn1)
            table.insert(antiConnections, conn1)
            
            -- Monitor new rooms
            local conn2 = rooms.ChildAdded:Connect(function(newRoom)
                task.spawn(function()
                    task.wait(0.2)
                    if antiEnabled.Eyefestation then
                        local interactables = newRoom:FindFirstChild("Interactables")
                        if interactables then
                            local eyefestationSpawn = interactables:FindFirstChild("EyefestationSpawn")
                            if eyefestationSpawn then
                                destroyInstance(eyefestationSpawn, "Eyefestation")
            ...
            table.insert(eyefestationConns, conn2)
            table.insert(antiConnections, conn2)
            
            -- Backup loop
            startLoop("EyefestationCheck", function()
                local rooms = workspace.GameplayFolder.Rooms
                for _, room in pairs(rooms:GetChildren()) do
                    local interactables = room:FindFirstChild("Interactables")
                    if interactables then
...
local AntiFirewallConn = nil
        AntiFeaturesSection:Toggle({
            Name = "Skip Firewall",
            Flag = "AntiFirewall",
            Default = false,
            Callback = function(Value)
            antiEnabled.Firewall = Value
        hasTeleported = false
        if Value then
            AntiFirewallConn = RunService.Heartbeat:Connect(function()
                if not antiEnabled.Firewall then return end
                local firewallEnd = rooms:FindFirstChild("FirewallEnd")
                if firewallEnd and not hasTeleported then
                    local door = firewallEnd.Entrances.LargeRoundDoor.Model
                    local char = player.Character
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        char.HumanoidRootPart.CFrame = door:GetPivot()
                        hasTeleported = true
                    end
                end
            end)
        else
            if AntiFirewallConn then AntiFirewallConn:Disconnect() end
        end
    end
        })
        ...
        -- Anti Abomination
        AntiFeaturesSection:Toggle({
            Name = "Anti Abomination",
            Flag = "AntiAbomination",
            Default = false,
            Callback = function(Value)
              antiEnabled.Abomination = Value
        if Value then
            scanAndDestroy(workspace, "Abomination", "Abomination")
            setupDescendantMonitor(workspace, "Abomination", "Abomination")
            startLoop("AbominationCheck", function()
                scanAndDestroy(workspace, "Abomination", "Abomination")
                local damageParts = workspace:FindFirstChild("GameplayFolder", true):FindFirstChild("Rooms", true):FindFirstChild("ChaseEnd", true):FindFirstChild("DamageParts", true)
                if damageParts then damageParts:Destroy() end
                local door = workspace:FindFirstChild("GameplayFolder", true):FindFirstChild("Rooms", true):FindFirstChild("ChaseEnd", true):FindFirstChild("Entrances", true):FindFirstChild("LargeRoundDoor", true)
            ...
            stopLoop("AbominationCheck")
        end
    end
        })
-- AntiVoid
local antiEnabled = { Void = false }
local descendantConnections = {}
local rooms = workspace.GameplayFolder.Rooms
local scanInterval = 0.3  -- Adjust scan frequency (seconds)

local function DestroyVoidLockers()
    for _, room in pairs(rooms:GetChildren()) do
        for _, descendant in pairs(room:GetDescendants()) do
            if descendant.Name == "MonsterLocker" then
                local prompt = descendant:FindFirstChild("ProximityPrompt")
                if prompt then
                    prompt:Destroy()
                    print("Destroyed Void Locker prompt in:", descendant:GetFullName())
                end
            end
        end
    end
...
local function SetupVoidMonitor(room)
    -- Monitor for new MonsterLockers in this room
    local connection = room.DescendantAdded:Connect(function(descendant)
        if descendant.Name == "MonsterLocker" then
            local prompt = descendant:FindFirstChild("ProximityPrompt")
            if prompt then
                prompt:Destroy()
                print("Destroyed new Void Locker prompt in:", descendant:GetFullName())
            end
        end
    end)
    table.insert(descendantConnections, connection)
    return connection
end

AntiFeaturesSection:Toggle({
    Name = "Anti Void Locker",
    Flag = "AntiVoid",
    Default = false,
    Callback = function(Value)
        antiEnabled.Void = Value
        if Value then
            -- Initial scan and destroy
            DestroyVoidLockers()

            -- Setup monitors for existing rooms
            for _, room in pairs(rooms:GetChildren()) do
                SetupVoidMonitor(room)
            end

            -- Monitor for new rooms
            local roomConn = rooms.ChildAdded:Connect(function(room)
                -- Immediately scan new room
                for _, descendant in pairs(room:GetDescendants()) do
                    if descendant.Name == "MonsterLocker" then
                        local prompt = descendant:FindFirstChild("ProximityPrompt")
                        if prompt then
                            prompt:Destroy()
                            print("Destroyed Void Locker prompt in new room:", descendant:GetFullName())
                        end
                    end
                end
                SetupVoidMonitor(room)
            end)
            table.insert(descendantConnections, roomConn)

            -- Start constant scanning loop
            spawn(function()
                while antiEnabled.Void do
                    DestroyVoidLockers()
                    wait(scanInterval)
                end
            end)
        ...
        -- Anti Mines and Tripwires
        -- Anti-Mine
local AntiMineValue = false
local function UpdateAllObjects(value)
    for _, room in pairs(rooms:GetChildren()) do
        for _, obj in pairs(room:GetDescendants()) do
            if obj.Name == "LandmineSpawn" then
                obj.CanTouch = not value
            elseif obj.Name == "Tripwire" and value then
                obj:Destroy()
            end
        end
    end
...
local function SetupObjectMonitoring(room)
    -- Process existing objects
    for _, obj in pairs(room:GetDescendants()) do
        if obj.Name == "LandmineSpawn" then
            obj.CanTouch = not AntiMineValue
        elseif obj.Name == "Tripwire" and AntiMineValue then
            obj:Destroy()
        end
    end
    ...
    local connection = room.DescendantAdded:Connect(function(descendant)
        if descendant.Name == "LandmineSpawn" then
            descendant.CanTouch = not AntiMineValue
        elseif descendant.Name == "Tripwire" and AntiMineValue then
            descendant:Destroy()
        end
    end)
    table.insert(descendantConnections, connection)
...
AntiFeaturesSection:Toggle({
    Name = "Anti Mines and Tripwires",
    Flag = "AntiMine",
    Default = false,
    Callback = function(Value)
        AntiMineValue = Value
        UpdateAllObjects(Value)

        if Value then
            -- Setup monitoring for existing rooms
            for _, room in pairs(rooms:GetChildren()) do
                SetupObjectMonitoring(room)
            end

            -- Monitor for new rooms
            local roomConn = rooms.ChildAdded:Connect(SetupObjectMonitoring)
            table.insert(descendantConnections, roomConn)

            -- Start constant scanning loop
            spawn(function()
                while AntiMineValue do
...
-- Anti-RemoteEvent (Trickster) - Optimized
local AntiRemoteEventValue = false
local descendantConnections = {}
local rooms = workspace.GameplayFolder.Rooms
local scanInterval = 0.3  -- Adjust scan frequency (seconds)

local function IsTargetRemoteEvent(obj)
    if not obj:IsA("RemoteEvent") then return false end
    local parent = obj.Parent
    if parent and parent.Name == "Trickster" then
        local grandparent = parent.Parent
        if grandparent and grandparent.Name == "Interactables" then
            return true
        end
    end
    return false
...
local function DestroyRemoteEvent(obj)
    if AntiRemoteEventValue and IsTargetRemoteEvent(obj) then
        obj:Destroy()
        print("Destroyed RemoteEvent in:", obj:GetFullName())
    end
end

local function FullRoomScan()
    -- Scan all existing rooms
    for _, room in pairs(rooms:GetChildren()) do
        local tricksterRoom = room:FindFirstChild("TricksterRoom")
        if tricksterRoom then
            local interactables = tricksterRoom:FindFirstChild("Interactables")
            if interactables then
                local trickster = interactables:FindFirstChild("Trickster")
                if trickster then
                    local remoteEvent = trickster:FindFirstChild("RemoteEvent")
                    if remoteEvent and IsTargetRemoteEvent(remoteEvent) then
                        remoteEvent:Destroy()
                        print("Destroyed RemoteEvent in:", remoteEvent:GetFullName())
                    end
                end
            end
        end
...
local function SetupRoomMonitor(room)
    -- Monitor for new RemoteEvents in this room
    local connection = room.DescendantAdded:Connect(function(obj)
        DestroyRemoteEvent(obj)
    end)
    table.insert(descendantConnections, connection)
    return connection
end
...
AntiFeaturesSection:Toggle({
    Name = "Anti Good People",
    Flag = "AntiRemoteEvent",
    Default = false,
    Callback = function(Value)
        AntiRemoteEventValue = Value
        if Value then
            -- Initial full scan
            FullRoomScan()
                ...
                if tricksterRoom then
                    ...
                    if interactables then
                        ...
                        if trickster then
                            ...
                            if remoteEvent and IsTargetRemoteEvent(remoteEvent) then
                                remoteEvent:Destroy()
                                print("Destroyed RemoteEvent in new room:", remoteEvent:GetFullName())
                            end
                        end
                    end
                end
...
local function setupDescendantMonitor(parent, targetNames, displayName, callback)
    ...
    local function monitor(descendant)
        ...
        elseif descendant.Name == targetNames then
            callback(descendant)
        end
    end

    -- Monitor existing descendants
    for _, child in pairs(parent:GetDescendants()) do
        monitor(child)
    end

    -- Monitor new descendants
    local conn = parent.DescendantAdded:Connect(monitor)
    table.insert(connections, conn)

    return connections
end

local function startConstantScan(parent, targetNames, displayName, callback, interval)
    -- Initial scan
    for _, child in pairs(parent:GetDescendants()) do
        if type(targetNames) == "table" then
            if targetNames[child.Name] then callback(child) end
        elseif child.Name == targetNames then callback(child) end
    end
...
-- Anti Turrets
local antiEnabled = { Turrets = false }
local turretConns = {}
local rooms = workspace.GameplayFolder.Rooms

local function destroyTurret(obj)
    if obj.Name == "Turret" then
        print("Destroyed Turret in:", obj:GetFullName())
        obj:Destroy()
    end
end
...
AntiFeaturesSection:Toggle({
    ...
    Default = false,
    Callback = function(Value)
        antiEnabled.Turrets = Value
        if Value then
            -- Scan and destroy existing turrets
            for _, room in pairs(rooms:GetChildren()) do
                local interactables = room:FindFirstChild("Interactables")
                if interactables then
                    for _, obj in pairs(interactables:GetDescendants()) do
                        destroyTurret(obj)
                    end
                    -- Monitor for new turrets
                    local conns = setupDescendantMonitor(interactables, "Turret", "Turrets", destroyTurret)
                    for _, conn in ipairs(conns) do table.insert(turretConns, conn) end
                end
            end
            ...
            table.insert(turretConns, roomConn)

            -- Start constant scanning
            startConstantScan(rooms, "Turret", "Turrets", destroyTurret, 2)
        else
            -- Cleanup
            for _, conn in ipairs(turretConns) do
...
-- Anti Searchlights
local AntiSearchlightsEnabled = false
local searchlightConns = {}
local searchlightNames = {
    MainSearchlight = true, Searchlights1 = true, Searchlights2 = true, Searchlights3 = true,
    SearchlightsEnding1 = true, SearchlightsEnding2 = true, SearchlightsEnding3 = true
}

local function destroySearchlight(obj)
    if searchlightNames[obj.Name] then
        print("Destroyed Searchlight:", obj.Name, "in:", obj:GetFullName())
...
AntiFeaturesSection:Toggle({
    ...
    Flag = "AntiSearchlights",
    Default = false,
    Callback = function(Value)
        AntiSearchlightsEnabled = Value
        if Value then
            -- Initial scan and destroy
            local conns = setupDescendantMonitor(rooms, searchlightNames, "Searchlights", destroySearchlight)
            for _, conn in ipairs(conns) do table.insert(searchlightConns, conn) end

            -- Start constant scanning
            startConstantScan(rooms, searchlightNames, "Searchlights", destroySearchlight, 0.5)
        else
            -- Cleanup
            for _, conn in ipairs(searchlightConns) do
                if conn.Disconnect then conn:Disconnect() end
            end
            searchlightConns = {}
        end
...
-- Anti Candles
local antiCandlesEnabled = false
local candleConns = {}
local monstersFolder = workspace:FindFirstChild("Monsters") or workspace

local function destroyCandle(obj, name)
    if obj and obj.Name == name then
        print("Destroyed", name, "in:", obj:GetFullName())
        pcall(function() obj:Destroy() end)
    end
end
...
AntiFeaturesSection:Toggle({
    ...
    Default = false,
    Callback = function(Value)
        antiCandlesEnabled = Value
        if Value then
            -- Initial destruction
            destroyCandle(monstersFolder:FindFirstChild("StatueRoot"), "StatueRoot")
            destroyCandle(workspace:FindFirstChild("BiggerStatue"), "BiggerStatue")
            for _, room in pairs(rooms:GetChildren()) do
                destroyCandle(room:FindFirstChild("StatueRoot"), "StatueRoot")
            end

            -- Setup monitors
            local conns1 = setupDescendantMonitor(monstersFolder, "StatueRoot", "Candles", function(obj) destroyCandle(obj, "StatueRoot") end)
            local conns2 = setupDescendantMonitor(rooms, "StatueRoot", "Candles", function(obj) destroyCandle(obj, "StatueRoot") end)
            local conns3 = setupDescendantMonitor(workspace, "BiggerStatue", "Candles", function(obj) destroyCandle(obj, "BiggerStatue") end)

            for _, conn in ipairs(conns1) do table.insert(candleConns, conn) end
            for _, conn in ipairs(conns2) do table.insert(candleConns, conn) end
            for _, conn in ipairs(conns3) do table.insert(candleConns, conn) end

            -- Start constant scanning
            spawn(function()
                while antiCandlesEnabled do
                    destroyCandle(monstersFolder:FindFirstChild("StatueRoot"), "StatueRoot")
                    destroyCandle(workspace:FindFirstChild("BiggerStatue"), "BiggerStatue")
                    for _, room in pairs(rooms:GetChildren()) do
                        destroyCandle(room:FindFirstChild("StatueRoot"), "StatueRoot")
            ...
            -- Cleanup
            for _, conn in ipairs(candleConns) do
                if conn.Disconnect then conn:Disconnect() end
            end
            candleConns = {}
        end
...
-- Anti Dweller
local antiDwellerEnabled = false
local dwellerConns = {}

local function destroyDweller(obj, name)
    if obj and (obj.Name == "DiVine" or obj.Name == "DiVineRoot") then
        print("Destroyed", name, "in:", obj:GetFullName())
        pcall(function() obj:Destroy() end)
    end
end
...
AntiFeaturesSection:Toggle({
    ...
    Default = false,
    Callback = function(Value)
        antiDwellerEnabled = Value
        if Value then
            -- Initial scan and destroy
            for _, room in pairs(rooms:GetChildren()) do
                destroyDweller(room:FindFirstChild("DiVine"), "DiVine")
            end
            destroyDweller(monstersFolder:FindFirstChild("DiVineRoot"), "DiVineRoot")

            -- Setup monitors
            local conns1 = setupDescendantMonitor(rooms, "DiVine", "Dweller", function(obj) destroyDweller(obj, "DiVine") end)
            local conns2 = setupDescendantMonitor(monstersFolder, "DiVineRoot", "Dweller", function(obj) destroyDweller(obj, "DiVineRoot") end)

            for _, conn in ipairs(conns1) do table.insert(dwellerConns, conn) end
            for _, conn in ipairs(conns2) do table.insert(dwellerConns, conn) end

            -- Start constant scanning
            spawn(function()
                while antiDwellerEnabled do
                    for _, room in pairs(rooms:GetChildren()) do
                        destroyDweller(room:FindFirstChild("DiVine"), "DiVine")
                    end
                    destroyDweller(monstersFolder:FindFirstChild("DiVineRoot"), "DiVineRoot")
            ...
            -- Cleanup
            for _, conn in ipairs(dwellerConns) do
                if conn.Disconnect then conn:Disconnect() end
            end
            dwellerConns = {}
        end
...
-- Anti Divine
local antiDivineEnabled = false
local divineConns = {}

local function destroyDivine(obj, name)
    if obj and (obj.Name == "DiVine" or obj.Name == "DiVineRoot") then
        print("Destroyed", name, "in:", obj:GetFullName())
        pcall(function() obj:Destroy() end)
    end
...
AntiFeaturesSection:Toggle({
    ...
    Default = false,
    Callback = function(Value)
        antiDivineEnabled = Value
        if Value then
            -- Initial scan and destroy
            for _, room in pairs(rooms:GetChildren()) do
                destroyDivine(room:FindFirstChild("DiVine"), "DiVine")
                destroyDivine(room:FindFirstChild("DiVineRoot"), "DiVineRoot")
            end

            -- Setup monitors for existing rooms
            for _, room in pairs(rooms:GetChildren()) do
                local conns = setupDescendantMonitor(room, {"DiVine", "DiVineRoot"}, "Divine", function(obj)
                    destroyDivine(obj, obj.Name)
                end)
                for _, conn in ipairs(conns) do table.insert(divineConns, conn) end
            end
            ...
            table.insert(divineConns, roomConn)

            -- Start constant scanning
            spawn(function()
                while antiDivineEnabled do
                    for _, room in pairs(rooms:GetChildren()) do
                        destroyDivine(room:FindFirstChild("DiVine"), "DiVine")
                        destroyDivine(room:FindFirstChild("DiVineRoot"), "DiVineRoot")
                    end
            ...
            -- Cleanup
            for _, conn in ipairs(divineConns) do
                if conn.Disconnect then conn:Disconnect() end
            end
            divineConns = {}
        end
...
-- Anti Baldi
local antiBaldiEnabled = false
local baldiConns = {}

local function destroyBaldi(obj)
    if obj and obj.Name == "Baldi" then
        print("Destroyed Baldi in:", obj:GetFullName())
        pcall(function() obj:Destroy() end)
    end
end
...
AntiFeaturesSection:Toggle({
    ...
    Default = false,
    Callback = function(Value)
        antiBaldiEnabled = Value
        if Value then
            -- Initial destruction
            destroyBaldi(workspace:FindFirstChild("Baldi"))

            -- Setup monitors
            local conns = setupDescendantMonitor(workspace, "Baldi", "Baldi", destroyBaldi)
            for _, conn in ipairs(conns) do table.insert(baldiConns, conn) end

            -- Start constant scanning
            spawn(function()
                while antiBaldiEnabled do
                    destroyBaldi(workspace:FindFirstChild("Baldi"))
                    wait(0.5)
                end
            end)
            ...
            -- Cleanup
            for _, conn in ipairs(baldiConns) do
                if conn.Disconnect then conn:Disconnect() end
            end
            baldiConns = {}
        end
    ...
    -- Automation Subpage
    local AutomationSubPage = HadalPage:SubPage({ Name = "Automation", Columns = 1 })

    do
        local AutomationSection = AutomationSubPage:Section({ Name = "Automation", Side = 1 })
    ...
    -- Try a named part like "Root" or "Base"
    local rootPart = obj:FindFirstChild("Root") or obj:FindFirstChild("Base")
    if rootPart and rootPart:IsA("BasePart") then
        return rootPart.Position
    end

    -- Fallback: Calculate centroid of all BasePart descendants
    local parts = {}
    ...
    if #parts == 0 then
        warn("No BaseParts found for Generator: " .. tostring(obj:GetFullName()))
        return nil
    end
...
local function fixGenerator(obj)
    pcall(function()
        -- Get generator's position
        local generatorPosition = getGeneratorPosition(obj)
        if not generatorPosition then
            warn("Could not determine position for Generator: " .. tostring(obj:GetFullName()))
            return
        end
        ...
        if not rootPart then
            print("No HumanoidRootPart found, skipping generator: " .. tostring(obj:GetFullName()))
            return
        end

        -- Calculate distance
        local distance = (rootPart.Position - generatorPosition).Magnitude
        print(string.format("Generator: %s, Player Position: %s, Generator Position: %s, Distance: %.2f studs",
            obj:GetFullName(), tostring(rootPart.Position), tostring(generatorPosition), distance))

        if distance > 10 then
            print("Generator too far (distance: " .. tostring(distance) .. " studs), skipping: " .. tostring(obj:GetFullName()))
            return
        end

        -- Invoke RemoteFunction if present
        if obj:FindFirstChild("RemoteFunction") then
            obj.RemoteFunction:InvokeServer()
            print("Invoked RemoteFunction on Generator: " .. tostring(obj:GetFullName()))
        end

        -- Fire RemoteEvent 20 times if present
        if obj:FindFirstChild("RemoteEvent") then
            for i = 1, 30 do
                obj.RemoteEvent:FireServer(true)
                task.wait(0.05) -- Small delay to avoid server overload
            end
            print("Fired RemoteEvent 20 times on Generator: " .. tostring(obj:GetFullName()))
        end
    end, function(err)
        warn("Failed to fix Generator: " .. tostring(obj:GetFullName()) .. ", Error: " .. tostring(err))
    end)
end
        -- Auto Fix Generators
        local AutoFixEnabled = false
        AutomationSection:Toggle({
            Name = "Auto Fix Generators",
            Flag = "AutoFix",
            Default = false,
            Callback = function(Value)
            AutoFixEnabled = Value
        if Value then
            print("AutoFix enabled, starting generator check loop...")
            task.spawn(function()
                while AutoFixEnabled do
                    local character = player.Character
                    local rootPart = character and character:FindFirstChild("HumanoidRootPart")
                    if rootPart then
                        -- Dynamically check for rooms
                        for _, room in pairs(rooms:GetChildren()) do
                                ...
                                if interactables then
                                    for _, obj in pairs(interactables:GetChildren()) do
                                        if obj.Name == "PresetGenerator" or obj.Name == "Generator" then
                                            print("Found generator: " .. tostring(obj:GetFullName()) .. ", checking distance...")
                                            fixGenerator(obj)
                                            task.wait(0.05) -- Wait before processing next generator
                                        end
                                    end
                                else
                                    print("No Interactables found in room: " .. tostring(room:GetFullName()))
                                end
                            end
                        end
                    else
                        print("No character or HumanoidRootPart found, skipping generator check")
                    end
                    task.wait(0.05) -- Check every 0.2 seconds for new/reset generators
                end
                print("AutoFix loop stopped")
            end)
        end
    end
        })
...
-- Auto-Hide
local function spawnAndTeleport(enemyName)
    ...
    if not safePart then
        ...
        safePart.Size = Vector3.new(10000, 1, 10000)
        safePart.Position = originalPosition.Position + Vector3.new(0, 400, 0)
    ...
    humanoidRootPart.CFrame = CFrame.new(safePart.Position + Vector3.new(0, safePart.Size.Y / 2 + 3, 0))
end
...
local function cleanup()
    ...
    angler = nil
        ...
        AutomationSection:Toggle({
            ...
            Default = false,
            Callback = function(Value)
             checkingEnemies = Value
        if Value then
            enemyCheckConn = RunService.Heartbeat:Connect(function()
                if not checkingEnemies then return end
                for _, child in pairs(workspace:GetChildren()) do
                    if entities[child.Name] then
                        if not angler or angler ~= child then
                            angler = child
                            spawnAndTeleport(child.Name)
                        end
                        if angler and safePart then
                            local character = player.Character
                            if character then
                                local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
            ...
            enemyRemoveConn = workspace.ChildRemoved:Connect(function(child)
                if child == angler and checkingEnemies then
                    cleanup()
                end
            end)
        else
            if enemyCheckConn then
                enemyCheckConn:Disconnect()
                enemyCheckConn = nil
            end
...
player.CharacterAdded:Connect(function()
    if checkingEnemies and angler and safePart then
        spawnAndTeleport(angler.Name)
    end
end)
...
local function tweenToTarget(rootPart, targetCFrame)
...
local function collectItem(prompt)
    ...
    collectedItems[prompt] = true
end

local function getKeycardCandidates(rootPart)
    local candidates = {}
    for _, child in pairs(workspace.GameplayFolder.Rooms:GetDescendants()) do
        if child:IsA("ProximityPrompt") and not collectedItems[child] then
            local proxyPart = child.Parent
            if proxyPart and proxyPart.Parent then
                local p = proxyPart.Parent
                if p.Name == "NormalKeyCard" or p.Name == "PasswordPaper" or p.Name == "InnerKeyCard" or p.Name == "RidgeKeyCard" then
                    local distance = (rootPart.Position - proxyPart.Position).Magnitude
                    if distance <= 5000 then
                        table.insert(candidates, {prompt = child, proxyPart = proxyPart, dist = distance})
                    end
                end
            end
        end
    ...
    return candidates
        ...
        if Value then
            loopThread = task.spawn(function()
                while true do
                    if isBusy or hiding or not player.Character then
                        task.wait(0.1)
                        continue
                    end
                    local rootPart = player.Character:WaitForChild("HumanoidRootPart")
                    local keyCandidates = getKeycardCandidates(rootPart)
                    if #keyCandidates > 0 then
                        table.sort(keyCandidates, function(a, b) return a.dist < b.dist end)
                        local target = keyCandidates[1]
                        isBusy = true
                        local targetPos = target.proxyPart.Position + Vector3.new(0, 3, 0)
                        local targetCFrame = CFrame.lookAt(targetPos, target.proxyPart.Position)
                        tweenToTarget(rootPart, targetCFrame)
                        if target.prompt:IsDescendantOf(workspace) then collectItem(target.prompt) end
                        task.wait(delay)
                        isBusy = false
                    else
                        local found = false
                        for _, room in pairs(workspace.GameplayFolder.Rooms:GetChildren()) do
                            local entrances = room:FindFirstChild("Entrances")
                            if entrances then
                                local door = entrances:FindFirstChild("NormalDoor")
                                if door and door:IsA("Model") and door.PrimaryPart and not tweenedDoors[door] then
                                    isBusy = true
                                    local doorCFrame = door.PrimaryPart.CFrame
                                    tweenToTarget(rootPart, doorCFrame)
                                    tweenedDoors[door] = true
                                    task.wait(delay)
                                    isBusy = false
                                    found = true
                                    break
...
do
    ...
    local RaveyardSection = RavePage:Section({ Name = "Anti-Features", Side = 1})
    local RaveyardSection2 = RavePage:Section({ Name = "Automation", Side = 1})
    -- Anti Bouncer
    RaveyardSection:Toggle({
        Name = "Anti Bouncer",
        Flag = "AntiSkeleton",
        Default = false,
        Callback = function(Value)
        antiEnabled.Bouncer = Value
        if Value then
            destroyInstance(monstersFolder:FindFirstChild("Bouncer"), "Bouncer")
            setupDescendantMonitor(monstersFolder, "Bouncer", "Bouncer")
            startLoop("BouncerCheck", function() scanAndDestroy(monstersFolder, "Bouncer", "Bouncer") end, 0.5)
        else
            stopLoop("BouncerCheck")
        end
    end
    })
    ...
    -- Anti Skeleton Head
    RaveyardSection:Toggle({
        Name = "Anti Skeleton Head",
        Flag = "AntiSkeleton2",
        Default = false,
        Callback = function(Value)
        antiEnabled.SkeletonHead = Value
        if Value then
            destroyInstance(monstersFolder:FindFirstChild("SkeletonHead"), "SkeletonHead")
            setupDescendantMonitor(monstersFolder, "SkeletonHead", "SkeletonHead")
            startLoop("SkeletonHeadCheck", function() scanAndDestroy(monstersFolder, "SkeletonHead", "SkeletonHead") end, 0.5)
        else
            stopLoop("SkeletonHeadCheck")
        end
    end
    })
...
local CandleConn = nil
    RaveyardSection:Toggle({
        ...
        Candle = Value
        if Value then
            CandleConn = RunService.Heartbeat:Connect(function()
                if not Candle then return end
                destroyInstance(monstersFolder:FindFirstChild("StatueRoot"), "StatueRoot")
                destroyInstance(workspace:FindFirstChild("BiggerStatue"), "BiggerStatue")
            end)
            destroyInstance(monstersFolder:FindFirstChild("StatueRoot"), "StatueRoot")
            destroyInstance(workspace:FindFirstChild("BiggerStatue"), "BiggerStatue")
            for _, room in pairs(rooms:GetChildren()) do
                destroyInstance(room:FindFirstChild("StatueRoot"), "StatueRoot")
            end
            setupDescendantMonitor(monstersFolder, "StatueRoot", "Candles")
            setupDescendantMonitor(rooms, "StatueRoot", "Candles")
            setupDescendantMonitor(workspace, "BiggerStatue", "Candles")
        else
            if CandleConn then CandleConn:Disconnect() end
        end
    end
    })
...
local delay = 0
local specialTweenDelay = .2 -- Delay for Raveyard2 tweens
local tweenedObjects = {}
local isBusy = false
local raveyard2Sequence = {
    "Entrances.GraveyardGate",
    "LowerCryptExit",
    "ChurchDoor",
    "Interactables.CryptDoor",
    "Interactables.ChurchDoor",
    "EndingStuff.PlayerTP"
}
local raveyard2Index = 1
local autoPlayEnabled = false
local raveyard2Found = false

-- Function to tween to a target CFrame
local function tweenToTarget(part, targetCFrame)
...
-- Function to find the next unvisited object in Entrances
local function findNextEntranceObject()
    for _, room in pairs(workspace.GameplayFolder.Rooms:GetChildren()) do
        local entrances = room:FindFirstChild("Entrances")
        if entrances then
            for _, object in pairs(entrances:GetChildren()) do
                if not tweenedObjects[object] then
                    -- Ensure PrimaryPart is set for models
                    if object:IsA("Model") and not object.PrimaryPart then
                        local primaryPart = object:FindFirstChildWhichIsA("BasePart")
                        if primaryPart then
                            object.PrimaryPart = primaryPart
                        else
                            warn("PrimaryPart not found for object in Entrances:", object.Name)
                            continue
                        end
                    end
                    return object
...
-- Function to get a target from Raveyard2 by path
local function getRaveyard2Target(path)
    local raveyard2 = workspace.GameplayFolder.Rooms:FindFirstChild("Raveyard2")
    if not raveyard2 then
        return nil
    end
    local target = raveyard2
...
-- Function to run the Raveyard2 sequence
local function runRaveyard2Sequence()
    if raveyard2Index > #raveyard2Sequence then
        warn("Raveyard2 sequence complete.")
        return false
    end

    local path = raveyard2Sequence[raveyard2Index]
    local target = getRaveyard2Target(path)
    if not target then
        warn("Target not found in Raveyard2:", path)
        raveyard2Index += 1
        return raveyard2Index <= #raveyard2Sequence
    end

    -- Ensure PrimaryPart is set for models
    if target:IsA("Model") and not target.PrimaryPart then
        local primaryPart = target:FindFirstChildWhichIsA("BasePart")
        if primaryPart then
            target.PrimaryPart = primaryPart
        else
            warn("PrimaryPart not found for target in Raveyard2:", target.Name)
            raveyard2Index += 1
            return raveyard2Index <= #raveyard2Sequence
        end
    end
    ...
    local rootPart = player.Character:WaitForChild("HumanoidRootPart")
    tweenToTarget(rootPart, targetPart.CFrame)
    task.wait(specialTweenDelay)
    raveyard2Index += 1
    return true
end
...
local function startAutoPlay()
    ...
    loopThread = task.spawn(function()
        while autoPlayEnabled do
            -- Check if Raveyard2 exists
            raveyard2Found = workspace.GameplayFolder.Rooms:FindFirstChild("Raveyard2") ~= nil

            if raveyard2Found then
                -- Raveyard2 found: pause Entrances, tween Raveyard2
                while autoPlayEnabled and raveyard2Found do
                    if not runRaveyard2Sequence() then
                        raveyard2Found = workspace.GameplayFolder.Rooms:FindFirstChild("Raveyard2") ~= nil
                        if not raveyard2Found then
                            warn("Raveyard2 disappeared, resuming Entrances...")
                            break
                        end
                        task.wait(1)
                    end
                ...
                -- Raveyard2 not found: tween Entrances
                if isBusy or not player.Character then
                    task.wait(0.1)
                    continue
                end

                local rootPart = player.Character:WaitForChild("HumanoidRootPart")
                local nextObject = findNextEntranceObject()

                if nextObject then
                    isBusy = true
                    local targetPart = nextObject:IsA("Model") and nextObject.PrimaryPart or nextObject
                    local targetCFrame = targetPart.CFrame
                    tweenToTarget(rootPart, targetCFrame)
                    tweenedObjects[nextObject] = true
                    task.wait(delay)
                    isBusy = false
                else
                    task.wait(0.5)
...
-- Auto Play Toggle
RaveyardSection2:Toggle({
    Name = "Auto Play",
    Flag = "AutoPlayRaveyard",
    Default = false,
    Callback = function(Value)
        autoPlayEnabled = Value
        if not Value and loopThread then
            task.cancel(loopThread)
            loopThread = nil
            isBusy = false
            tweenedObjects = {}
            raveyard2Index = 1
            raveyard2Found = false
        else
            startAutoPlay()
        end
    end
...
-- Sliders for tween speed and delay
RaveyardSection2:Slider({
    Name = "Tween Speed (studs/s)",
    Flag = "TweenSpeedRaveyard",
    Min = 100,
    Max = 800,
    Default = 200,
    Decimals = 100,
...
-- The Hunted Page
do
    local HuntedSection = HuntedPage:Section({ Name = "Anti-Features", Side = 1 })

    -- Add Hunted-specific features here
    HuntedSection:Toggle({
        Name = "Anti Cropse & Pipsqueak",
        Flag = "AntiHuntedFeature",
        Default = false,
        Callback = function(Value)
            -- Implement Anti Hunted Feature logic
        end
    })
end
...
-- Visual Page
do
    local VisualsSection = VisualPage:Section({ Name = "Visuals", Side = 1 })
...
local function checkForEntities()
    for name, _ in pairs(entities) do
        if workspace:FindFirstChild(name) then
            Library:Notification(name.. " has been spawned!", "-", 5)
        end
    end
end
    ...
    Callback = function(Value)
        if Value then
            ...
            connection = workspace.ChildAdded:Connect(function(child)
                if entities[child.Name] then
                    Library:Notification(child.Name.. " has been spawned!", "-", 15)
                end
            end)
            -- Store connection for later disconnection
            getgenv().entityNotifierConnection = connection
        else
            -- Disconnect the event when toggled off
            if getgenv().entityNotifierConnection then
                getgenv().entityNotifierConnection:Disconnect()
                getgenv().entityNotifierConnection = nil
            end
        end
    end
})
...
local ESPConfigs = {
    {
        ...
        matcher = function(obj)
            return obj:GetAttribute("InteractionType") == "ItemBase"
        end,
        folderNames = {"Rooms", "DroppedItems"},
    ...
    },
    {
        ...
        matcher = function(obj)
            return obj:GetAttribute("InteractionType") == "CurrencyBase"
        end,
        folderNames = {"Rooms", "DroppedItems"},
        isWorkspaceScan = false,
    },
    ...
    },
    {
        ...
        matcher = function(obj)
            local monsters = {
                ...
                RidgeFroger = true,
                RidgeBlitz = true,
                RidgeChainsmoker = true,
                RidgePandemonium = true
            }
            return monsters[obj.Name]
        end,
        folderNames = {"Rooms"},
    ...
    {
        name = "TricksterDoors",
        flag = "TricksterDoorESP",
        text = "ESP Trickster Doors",
        default = false,
        color = Color3.fromRGB(255, 40, 0),
        matcher = function(obj)
            return obj.Name == "TricksterDoor"
        end,
        folderNames = {"Rooms"},
        isWorkspaceScan = false,
    },
    {
        ...
        matcher = function(obj)
            return obj.Name == "Generator" or obj.Name == "PresetGenerator"
        end,
        folderNames = {"Rooms"},
        isWorkspaceScan = false,
    },
    {
        ...
        text = "ESP Doors",
        default = false,
        color = Color3.fromRGB(100, 230, 20),
        matcher = function(obj)
            return obj.Name == "NormalDoor" and obj.Parent and obj.Parent.Name == "Entrances" and obj.Parent.Parent.Parent == rooms
        end,
        folderNames = {"Rooms"},
        isWorkspaceScan = false,
    },
...
local lastUpdateTime = 0
local espDistances = {}
local droppedItemsFolder = gameplayFolder:WaitForChild("DroppedItems")
...
local function getObjectPosition(object)
    ...
    elseif object:IsA("Model") then
        return object.PrimaryPart and object.PrimaryPart.Position or
               (object:FindFirstChild("Handle") or object:FindFirstChildWhichIsA("BasePart")) and
               (object:FindFirstChild("Handle") or object:FindFirstChildWhichIsA("BasePart")).Position or
               object:GetPivot().Position
    end
    return nil
end

local function createBillboardGui(parent, color, distance)
    if not parent or not parent:IsDescendantOf(workspace) or parent:FindFirstChild("ESPBillboard") then
        return
    end
    local adornee = parent:IsA("BasePart") and parent or parent.PrimaryPart or parent:FindFirstChild("Handle") or parent:FindFirstChildWhichIsA("BasePart")
    if not adornee then
        return
    end
    ...
    billboard.MaxDistance = distance or 100
    ...
    billboardCache[parent] = billboard
    parent.AncestryChanged:Connect(function()
        if not parent:IsDescendantOf(workspace) and billboardCache[parent] then
            billboardCache[parent]:Destroy()
            billboardCache[parent] = nil
        end
    end)
end

local function removeBillboards(matcher)
    for item, billboard in pairs(billboardCache) do
        if matcher(item) and billboard then
            billboard:Destroy()
            billboardCache[item] = nil
        end
    end
end
...
local function updateDistanceDisplay()
    ...
    for item, billboard in pairs(billboardCache) do
        if billboard and billboard.Parent then
            ...
            if textLabel then
                local itemPosition = getObjectPosition(item)
                if itemPosition then
                    local dist = (rootPart.Position - itemPosition).Magnitude
                    local displayName = item.Name
                    if item.Name == "Generator" or item.Name == "PresetGenerator" then
                        local fixed = item:FindFirstChild("Fixed")
                        local fixedValue = fixed and (fixed:IsA("IntValue") or fixed:IsA("NumberValue")) and fixed.Value or "N/A"
                        textLabel.Text = string.format("%s\nFixed: %s\n%.1f studs", displayName, tostring(fixedValue), dist)
                    else
                        textLabel.Text = string.format("%s\n%.1f studs", displayName, dist)
                    end
                else
...
local function scanFolder(folder, matcher, color, distance)
    if not folder then
        return
    end
    for _, descendant in pairs(folder:GetDescendants()) do
        if matcher(descendant) then
            createBillboardGui(descendant, color, distance)
        end
    end
end

local function isAnyESPEabled()
    for _, config in ipairs(ESPConfigs) do
        if Library.Flags[config.flag] then
            return true
        end
    end
    return false
...
local function initializeESP(config)
    if not Library.Flags[config.flag] then
        return
    end
    local distance = espDistances[config.name] or 100
    if config.isWorkspaceScan then
        for _, child in pairs(workspace:GetChildren()) do
            if config.matcher(child) then
                createBillboardGui(child, config.color, distance)
            end
        end
        workspace.ChildAdded:Connect(function(child)
            if Library.Flags[config.flag] and config.matcher(child) then
                createBillboardGui(child, config.color, espDistances[config.name] or 100)
            end
        end)
    end
    for _, folderName in ipairs(config.folderNames) do
        local folder = folderName == "Rooms" and rooms or droppedItemsFolder
        if folder then
            scanFolder(folder, config.matcher, config.color, distance)
            folder.DescendantAdded:Connect(function(descendant)
                task.wait(0.1)
                if Library.Flags[config.flag] and config.matcher(descendant) then
                    createBillboardGui(descendant, config.color, espDistances[config.name] or 100)
                end
            end)
        end
    end
...
-- Initialize ESP for each config
for _, config in ipairs(ESPConfigs) do
    espDistances[config.name] = 350
    VisualsSection:Toggle({
        Name = config.text,
        Flag = config.flag,
        ...
        Callback = function(Value)
            if Value then
                initializeESP(config)
                if not heartbeatConnection and isAnyESPEabled() then
                ...
                removeBillboards(config.matcher)
                if not isAnyESPEabled() then
                    removeAllBillboards()
...
-- Movement Page
do
    local MovementSection = MovementPage:Section({ Name = "Movement", Side = 1 })
...
local speedConn = nil
    ...
    MovementSection:Slider({
        ...
        Decimals = 10,
        Callback = function(Value)
             player:SetAttribute("CustomSpeed", Value)
        end
    })
...
local function updateMovement()
    ...
    if hum.MoveDirection.Magnitude > 0 then
        local speed = player:GetAttribute("CustomSpeed") or 0
        local delta = RunService.Heartbeat:Wait()
        local moveVector = hum.MoveDirection * speed * delta
        root.CFrame = root.CFrame + Vector3.new(moveVector.X, 0, moveVector.Z)
    end
end

speedConn = RunService.Heartbeat:Connect(updateMovement)

local Players = game:GetService("Players")
local player = Players.LocalPlayer
local activeLoops = {}
local selectedLootTypes = { Currency = false, KeyCards = false, Items = false }

-- Function to find and fire ProximityPrompts for selected loot types
local function findAndFireProximityPrompts()
    local character = player.Character or player.CharacterAdded:Wait()
    if not character then return end
    ...
    if not roomsFolder then
        warn("Rooms folder not found")
        return
    end

    for _, room in ipairs(roomsFolder:GetChildren()) do
        for _, object in ipairs(room:GetDescendants()) do
            if object:IsA("ProximityPrompt") and object:FindFirstAncestor("SpawnLocations") then
                local proxyPart = object.Parent
                if proxyPart and proxyPart:IsA("BasePart") then
                    ...
                    if p.Name == "NormalKeyCard" or p.Name == "PasswordPaper" or p.Name == "InnerKeyCard" or p.Name == "RidgeKeyCard" then
                        lootType = "KeyCards"
                    elseif p:GetAttribute("InteractionType") == "CurrencyBase" then
                        lootType = "Currency"
                    elseif p:GetAttribute("InteractionType") == "ItemBase" or p:GetAttribute("InteractionType") == "Ammo" then
                        lootType = "Items"
                    end

                    if lootType and selectedLootTypes[lootType] then
                        local distance = (rootPart.Position - proxyPart.Position).Magnitude
                        if distance <= 12 then
                            while object:IsDescendantOf(workspace) and (rootPart.Position - proxyPart.Position).Magnitude <= 12 do
...
local function startLoop(name, callback, interval)
    ...
    activeLoops[name .. "_thread"] = task.spawn(function()
        while activeLoops[name] do
            callback()
            task.wait(interval or 0.1)
        end
...
local function stopLoop(name)
    if activeLoops[name] then
        ...
        if activeLoops[name .. "_thread"] then
            task.cancel(activeLoops[name .. "_thread"])
            activeLoops[name .. "_thread"] = nil
        end
    end
end

-- Function to start the loot loop
local function startLootLoop()
    -- Check if any loot type is selected
    local anySelected = selectedLootTypes.Currency or selectedLootTypes.KeyCards or selectedLootTypes.Items

    if anySelected then
        startLoop("AutoLoot", findAndFireProximityPrompts, 0.1)
        print("Auto Loot loop started")
    else
        print("Auto Loot not started (nothing selected)")
    end
end
...
MovementSection:Toggle({
    ...
    Callback = function(Value)
        if not Value then
            stopLoop("AutoLoot")
            print("Auto Loot disabled by toggle")
        else
            startLootLoop()
        end
    end
...
-- Dropdown to select what to loot
MovementSection:Dropdown({
    Name = "Auto Loot Aura",
    Flag = "AutoLoot",
    Items = {"Currency", "KeyCards", "Items"},
    Multi = true,
    Callback = function(Values)
        -- Reset selected loot types
        selectedLootTypes = { Currency = false, KeyCards = false, Items = false }

        -- Debug: Print the Values table to understand its structure
        print("Dropdown Values:", Values)

        -- Set selected loot types based on Values
        for _, item in ipairs(Values) do
            selectedLootTypes[item] = true
        end

        print("Selected Loot Types:", selectedLootTypes)

        -- Start or stop the loop based on selections and toggle state
        if Library.Flags.AutoLootToggle then
            stopLoop("AutoLoot")
            startLootLoop()
        end
    end
...
-- ProximityPrompt + Password Paper + Keypad Entry (Final)
local player = game:GetService("Players").LocalPlayer
local rooms = workspace.GameplayFolder.Rooms

local function findPasswordCode()
    print("Scanning for PasswordPaper...")
    for _, room in pairs(rooms:GetChildren()) do
        for _, obj in pairs(room:GetDescendants()) do
            if obj.Name == "PasswordPaper" then
                local codeObj = obj:FindFirstChild("Code")
                if codeObj then
                    local surfaceGui = codeObj:FindFirstChild("SurfaceGui")
                    if surfaceGui then
                        local textLabel = surfaceGui:FindFirstChild("TextLabel")
                        if textLabel then
                            local code = textLabel.Text
    ...
    print("No PasswordPaper or code found.")
    return nil
end

local function findAndFireKeypad(code)
    print("Scanning for Keypad ClickDetectors...")
    local keypad = {}
    for _, room in pairs(rooms:GetChildren()) do
        for _, obj in pairs(room:GetDescendants()) do
            if obj.Name:match("Keypad%d") or obj.Name == "KeypadBackspace" then
                local clickDetector = obj:FindFirstChild("ClickDetector")
                if clickDetector then
                    keypad[obj.Name] = clickDetector
    ...
    if next(keypad) == nil then
        print("No Keypad ClickDetectors found.")
        return false
    end
    ...
    for i = 1, #code do
        ...
        local keypadName = "Keypad" .. digit
        if keypad[keypadName] then
            print("Firing:", keypadName)
            fireclickdetector(keypad[keypadName])
            wait(0) -- Minimal delay between key presses
        else
            print("Keypad" .. digit .. " not found!")
        end
    end
    return true
end
...
MovementSection:Toggle({
    Name = "Insta Door Open",
    Flag = "ProximityKeypad",
    Default = false,
}):Keybind({
    Flag = "ProximityKeypadKeybind",
    Default = Enum.KeyCode.E,
    Mode = "Hold",
    Callback = function(Value)
        if Value then
            print("--- Starting Proximity + Keypad Auto-Entry ---")

            -- 1. Fire nearest ProximityPrompt (FIRST PRIORITY)
            local char = player.Character
            if not char then return end
            local root = char:FindFirstChild("HumanoidRootPart")
            ...
            for _, room in pairs(rooms:GetChildren()) do
                for _, prompt in pairs(room:GetDescendants()) do
                    if prompt:IsA("ProximityPrompt") then
                        ...
                        if (parent.Name == "Main" or parent.Name == "ProxyPart") and parent:IsA("BasePart") then
                            local dist = (root.Position - parent.Position).Magnitude
                            if dist <= 12 and dist < closestDist then
                                closestDist = dist
            ...
            if closestPrompt then
                print("Firing ProximityPrompt:", closestPrompt:GetFullName())
            ...
            -- 2. Scan for code and enter it immediately
            local code = findPasswordCode()
            if not code then return end

            -- 3. Scan for and fire Keypad ClickDetectors
            findAndFireKeypad(code)
...
-- Function to find and fire ProximityPrompts in Entrances
local function findAndFireProximityPrompts()
    ...
    for _, room in ipairs(rooms:GetChildren()) do
        local entrances = room:FindFirstChild("Entrances")
        if entrances then
            for _, entrance in ipairs(entrances:GetChildren()) do
...
MovementSection:Toggle({
    Name = "Force Door Open [Good for autoplays]",
    Flag = "Door",
    Default = false,
    Callback = function(Value)
        if Value then
            -- Start the loop if the toggle is enabled
            loopThread = task.spawn(function()
                while Library.Flags.Door do
                    findAndFireProximityPrompts()
                    task.wait(0.01) -- Adjust delay as needed
                end
            end)
        else
            -- Cancel the loop if the toggle is disabled
            if loopThread then
                task.cancel(loopThread)
                loopThread = nil
            end
...
local afterDeathGui = mainGui:WaitForChild("AfterDeath")

-- Function to fire the PlayAgain event once
local function firePlayAgainOnce()
    local playAgainEvent = ReplicatedStorage:FindFirstChild("Events") and ReplicatedStorage.Events:FindFirstChild("PlayAgain")
    if playAgainEvent then
        playAgainEvent:FireServer()
    else
        warn("PlayAgain event not found in ReplicatedStorage.Events")
    end
end

-- Toggle for Auto-Retry
MovementSection:Toggle({
    Name = "Auto Retry",
    Flag = "Retry",
    Default = false,
    Callback = function(Value)
        if Value then
            -- Connect to the Visible property change signal
            connection = afterDeathGui:GetPropertyChangedSignal("Visible"):Connect(function()
                if afterDeathGui.Visible then
                    firePlayAgainOnce()
            ...
            -- Disconnect the signal if the toggle is disabled
            if connection then
                connection:Disconnect()
                connection = nil
            end
